...
export function drawWalls(ctx, player, canvas) {
    ...
    const starCount = 100;
    const starAnimationTime = Date.now() * 0.001;
    for (let i = 0; i < starCount; i++) {
        const x = Math.sin(i * 567.5) * canvas.width;
        const y = Math.cos(i * 321.7) * canvas.height/2;
        const twinkle = Math.sin(starAnimationTime + i * 0.5) * 0.5 + 0.5;
    ...
    for (let i = 0; i < numRays; i++) {
        ...
        const colorIndex = (colorCycleOffset + Math.floor(i / (numRays / colorMaps.length))) % colorMaps.length;
